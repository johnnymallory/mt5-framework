...
class DataProvider():
    ...
    def get_bars_from_range(self, symbol:str, timeframe: str, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """
        Retrieves the closed bars for a given symbol and timeframe in a specified period of time.

        Args:
            symbol (str): The symbol to retrieve bars for.
            timeframe (str): The timeframe of the bars (e.g., 'M1', 'H1', 'D1').
            start_date (datetime): The start of the range we will retrieve the bars from
            end_date (datetime): The end of the range we will retrieve the bars from

        Returns:
            pd.DataFrame: A DataFrame containing the range's closed bars data.
        ...
        tf = self._map_timeframes(timeframe)
        try:
            bars_np_array = mt5.copy_rates_range(symbol, tf, start_date, end_date)
            ...
            print(f"{Utils.dateprint()} - No se han podido recuperar los datos del rango de velas de {symbol} {timeframe} - MT5 Error: {mt5.last_error()}, exception: {e}")
