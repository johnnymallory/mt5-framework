...
from events.events import DataEvent, SignalEvent, SizingEvent, OrderEvent, ExecutionEvent, PlacedPendingOrderEvent, UpdateEvent
...
class TradingDirector():
    ...
    def _handle_update_event(self, event: UpdateEvent):
        """
        Handle the update order event.

        Args:
            event (UpdateEvent): The order event object.

        Returns:
            None
        """
        print(f"{Utils.dateprint()} - Recibido UPDATE EVENT en {event.symbol}")
        self.ORDER_EXECUTOR._update_sltp_in_position_by_ticket(event.ticket)
