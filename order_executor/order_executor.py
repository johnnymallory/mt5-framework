...
class OrderExecutor():
    ...
    def _update_sltp_in_position_by_ticket(self, ticket: int) -> None:
        """
        Updates a position by ticket number
        
        Args:
        ticket (int): Position ticket. Unique number assigned to each newly opened position. It usually matches the ticket of an order used to open the position except when the ticket is changed as a result of service operations on the server, for example, when charging swaps with position re-opening. To find an order used to open a position, apply the POSITION_IDENTIFIER property

        Returns:
        None
        """
        
        # Accedemos a la posición abierta 
        position = mt5.position_get(ticket)[0]
        
        # Evaluar la posición
        if position is None:
            print(f"{Utils.dateprint()} - ORDER EXECUTOR ERROR: No existe ninguna posición abierta con el ticket {ticket}")
            return
        
        # Creación del trade request para actualizar la posición
        update_request = {
            'action': mt5.TRADE_ACTION_DEAL,
            'position': position.ticket,
            'symbol': position.symbol,
        ...
        # Mandar el close_request
        result = mt5.order_send(update_request)
        
        # Evaluar el resultado
        if self._check_execution_status(result):
            print(f"{Utils.dateprint()} - Actualizado SLTP en {position.symbol} con volumen {position.volume} y ticket {ticket}.")
            self._create_and_put_execution_event(result)
        else:
            print(f"{Utils.dateprint()} - Ha habido un error al actualizar el SLTP de la posición {ticket} en {position.symbol}: {result.comment}")
